...
contract UpgradeToAlignedTokenV2 is Script {
    function run(string memory config) public {
        ...
        address _currentClaimableAirdropProxy = stdJson.readAddress(
            config_json,
            ".claimableProxy"
        );

        vm.broadcast();
        ExampleAlignedTokenV2 _newClaimable = new ExampleAlignedTokenV2();

        bytes memory _upgradeCalldata = abi.encodeCall(
            ProxyAdmin.upgradeAndCall,
            (
                ITransparentUpgradeableProxy(_currentClaimableAirdropProxy),
                address(_newClaimable),
        ...
        console.log(
            "Proxy Admin to call:",
            getAdminAddress(_currentClaimableAirdropProxy)
        );
    }

...
func (s *AvsSubscriber) SubscribeToNewTasksV2(newTaskCreatedChan chan *servicemanager.ContractAlignedLayerServiceManagerNewBatchV2) (chan error, error) {
	...
	sub, err := SubscribeToNewTasksV2Retryable(&bind.WatchOpts{}, s.AvsContractBindings.ServiceManager, internalChannel, nil, retry.EthCallRetryParams())
	if err != nil {
		s.logger.Error("Primary failed to subscribe to new AlignedLayer V2 tasks after %d retries", retry.EthCallNumRetries, "err", err)
		return nil, err
	}

	subFallback, err := SubscribeToNewTasksV2Retryable(&bind.WatchOpts{}, s.AvsContractBindings.ServiceManagerFallback, internalChannel, nil, retry.EthCallRetryParams())
	if err != nil {
...
func (s *AvsSubscriber) SubscribeToNewTasksV3(newTaskCreatedChan chan *servicemanager.ContractAlignedLayerServiceManagerNewBatchV3) (chan error, error) {
	...
	// Subscribe to new tasks
	sub, err := SubscribeToNewTasksV3Retryable(&bind.WatchOpts{}, s.AvsContractBindings.ServiceManager, internalChannel, nil, retry.EthCallRetryParams())
	if err != nil {
		s.logger.Error("Primary failed to subscribe to new AlignedLayer V3 tasks after %d retries", MaxRetries, "err", err)
		return nil, err
	}

	subFallback, err := SubscribeToNewTasksV3Retryable(&bind.WatchOpts{}, s.AvsContractBindings.ServiceManagerFallback, internalChannel, nil, retry.EthCallRetryParams())
	if err != nil {
...
func (s *AvsSubscriber) getLatestNotRespondedTaskFromEthereumV2() (*servicemanager.ContractAlignedLayerServiceManagerNewBatchV2, error) {
	...
	logs, err := s.FilterBatchV2Retryable(&bind.FilterOpts{Start: fromBlock, End: nil, Context: context.Background()}, nil, retry.EthCallRetryParams())
	if err != nil {
		return nil, err
	}
	...
	state, err := s.BatchesStateRetryable(nil, batchIdentifierHash, retry.EthCallRetryParams())
	if err != nil {
		return nil, err
	}
...
func (s *AvsSubscriber) getLatestNotRespondedTaskFromEthereumV3() (*servicemanager.ContractAlignedLayerServiceManagerNewBatchV3, error) {
	...
	logs, err := s.FilterBatchV3Retryable(&bind.FilterOpts{Start: fromBlock, End: nil, Context: context.Background()}, nil, retry.EthCallRetryParams())
	if err != nil {
		return nil, err
	}
...
func (s *AvsSubscriber) WaitForOneBlock(startBlock uint64) error {
	currentBlock, err := s.BlockNumberRetryable(context.Background(), retry.EthCallRetryParams())
	if err != nil {
		return err
	}
	...
	if currentBlock <= startBlock { // should really be == but just in case
		...
		sub, err := s.SubscribeNewHeadRetryable(context.Background(), c, retry.EthCallRetryParams())
		if err != nil {
			return err
		}
